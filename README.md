...
> [!NOTE]
> I nodi **Edge** sono volutamente **indipendenti** dal cloud stack (Keycloak, Service Gateway). Questo permette di dimostrare il comportamento **offline-first**: gli edge continuano a ricevere partite via MQTT e a bufferizzarle localmente in SQLite anche se il cloud è irraggiungibile. La sincronizzazione riprende automaticamente al ripristino della connettività.
...
| **3** | **`service-gateway`** | Spring Cloud Gateway | • Agisce da API Gateway / Reverse Proxy centrale sulla porta `8081`.<br>• Applica la verifica dei token OIDC e il filtro `TenantVerificationGatewayFilterFactory` per il controllo dell'header/claim `locale_id`.<br>• Inoltra il traffico verso `partita-service`, `torneo-service` e `statistiche-service`.<br>• Espone la documentazione **Swagger UI** (`/docs`) e la **Dashboard Centrale** (`/dashboard`). |
...
| `http://localhost:8081/docs` | **Documentazione API Swagger UI** | Interfaccia Swagger/OpenAPI 3.0 interattiva per consultare, esplorare e collaudare gli endpoint REST esposti dal Service Gateway. |
