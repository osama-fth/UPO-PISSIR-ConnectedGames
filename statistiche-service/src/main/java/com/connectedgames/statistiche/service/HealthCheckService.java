...
public class HealthCheckService {
    ...
    public HealthStatusResponse getSystemHealth() {
        ...
        cloudServices.add(checkCloudService("Service Gateway", "API Gateway & Reverse Proxy",
                "http://service-gateway:8081/actuator/health"));
        cloudServices.add(checkCloudService("Partita Service", "Gestione & Bulk Sync Partite",
                "http://partita-service:8082/actuator/health"));
        cloudServices.add(checkCloudService("Torneo Service", "Gestione Tornei & Classifiche",
                "http://torneo-service:8083/actuator/health"));
        cloudServices.add(checkCloudService("Statistiche Service", "BFF Analytics & Super Admin UI",
                "http://127.0.0.1:8084/actuator/health"));
        cloudServices.add(checkKeycloak("Keycloak IdP", "OAuth2 / OIDC Identity Provider",
                "http://keycloak:8080/realms/Connected-Games"));
    ...
    private ServizioCloudStatus checkCloudService(String nome, String ruolo, String url) {
        ...
        try {
            ...
            String status = (response != null && "UP".equalsIgnoreCase(String.valueOf(response.get("status")))) ? "UP"
                    : "DEGRADED";
    ...
    private ServizioCloudStatus checkKeycloak(String nome, String ruolo, String url) {
        ...
        try {
            ...
            return new ServizioCloudStatus(nome, ruolo, url, isOk ? "UP" : "DOWN", latency,
                    isOk ? "Realm active" : "Realm non attivo");
        } catch (Exception e) {
            long latency = System.currentTimeMillis() - start;
            return new ServizioCloudStatus(nome, ruolo, url, "DOWN", latency,
                    "Keycloak non raggiungibile: " + e.getMessage());
        }
    }
    ...
    private ServizioCloudStatus checkPlatformDatabase() {
        ...
        String url = System.getenv().getOrDefault("SPRING_DATASOURCE_URL",
                "jdbc:postgresql://postgres-db:5432/platform_db");
        ...
        try (Connection conn = DriverManager.getConnection(url, user, pass)) {
            ...
            long latency = System.currentTimeMillis() - start;
            return new ServizioCloudStatus("Platform Database", "Database Piattaforma", url, isOk ? "UP" : "DOWN",
                    latency, isOk ? "Connessione OK" : "DB non valido");
        } catch (Exception e) {
            long latency = System.currentTimeMillis() - start;
            return new ServizioCloudStatus("Platform Database", "Database Piattaforma", url, "DOWN", latency,
    ...
    private ServizioCloudStatus checkKeycloakDatabase() {
        ...
        try (Connection conn = DriverManager.getConnection(url, user, pass)) {
            boolean isOk = conn.isValid(2);
            long latency = System.currentTimeMillis() - start;
            return new ServizioCloudStatus("Keycloak Database", "Database Identity Provider", url, isOk ? "UP" : "DOWN",
                    latency, isOk ? "Connessione OK" : "DB non valido");
        } catch (Exception e) {
            long latency = System.currentTimeMillis() - start;
            return new ServizioCloudStatus("Keycloak Database", "Database Identity Provider", url, "DOWN", latency,
                    "Errore DB: " + e.getMessage());
        }
    }
}
