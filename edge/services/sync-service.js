...
async function getServiceAccountToken() {
    ...
    try {
        ...
        cachedServiceTokenExpiresAt = claims.exp ? (claims.exp * 1000) - 30000 : Date.now() + (55 * 60 * 1000);
    } catch {
...
// Invia in modalità bulk le partite non ancora sincronizzate al Server Centrale
async function sincronizzaAdesso(accessToken) {
    ...
    try {
        ...
        let tokenToUse = accessToken || await getServiceAccountToken();
        const headers = {
        ...
        if (fallite.length > 0) {
            fallite.forEach(f => console.warn(`[Sync ${LOCALE_ID}] Partita ${f.id} fallita: ${f.errore}`));
        }

        lastSyncTime = new Date().toISOString();
        lastSyncResult = { salvate: salvateIds.length, fallite: fallite.length, dettagliFallite: fallite };
...
// Pianifica la sincronizzazione automatica ogni 2 minuti
