...
router.get('/callback', async (req, res) => {
    try {
        ...
        // Controllo Sicurezza: Solo admin_piattaforma per la Dashboard Piattaforma
        if (!userInfo.roles.includes('admin_piattaforma')) {
            console.error(`[Platform Dashboard] Accesso negato: utente ${userInfo.username} non ha ruolo admin_piattaforma`);
            
            const dashboardUrl = process.env.DASHBOARD_URL || `http://localhost:${process.env.PORT || 3000}`;
            const logoutUrl = getLogoutUrl(
                tokenSet.id_token,
                `${dashboardUrl}/auth/login?error=unauthorized_admin`
            );
